...
def TmpData():            
    with open('/tmp/curl_tmp.txt','w') as f:  # 写文件
        for merit_k in merit.keys():
            f.write("%s,%s,%s,%s,%s\n"%(merit_k,merit[merit_k][3],merit[merit_k][6],merit[merit_k][7],merit[merit_k][0]))
def ReadDic():
    with open('/tmp/curl_tmp.txt') as f:  # 转换字典
        for l in f.xreadlines():  
            l = l.split(',')
            up_merit[l[0]] = l[1],l[2],l[3],l[4]
len_num = LenNum()
ChangeMerit()
# TmpData()
ReadDic()
...
for merit_k in merit.keys(): # 两个字典的K一样，因此做一次循环
    if int(merit[merit_k][0]) < int(up_merit[merit_k][3]):
        break 
    ...
    if float(req_total_sum) < 100.0:continue # 总请求太低不计算
